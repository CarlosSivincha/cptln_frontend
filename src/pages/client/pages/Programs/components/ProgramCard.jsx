...
import { obtenerCategoriasID } from "@/Api/categorias";
    ...
    return (
        <div className={`mx-10 sm:mx-auto md:mx-10 min-[930px]:mx-auto max-w-full max-md:max-w-[560px] max-lg:max-w-[800px] min-[880px]:mx-auto lg:mx-20 md:flex md:h-[350px] lg:h-[500px] min-[1100px]:h-[550px] xl:h-[650px] 2xl:h-[750px] max-[1450px]:max-w-[1380px] xl:mx-24 min-[1700px]:mx-auto min-[1700px]:w-[1600px] min-[1800px]:w-[1550px] lg:relative`}>
            <div className="md:flex md:w-1/2 lg:w-full">
                <div
                    className={`h-full lg:w-2/6 max-lg:hidden ${posicion === "izquierda" ? "order-1 lg:rounded-l-[12px]" : "order-2 lg:rounded-r-[12px]"}`}
                    style={{
                        backgroundColor: props.color,
                    }}
                ></div>

                <div className={`relative max-sm:h-48 md:h-full object-cover lg:w-4/6 ${posicion === "izquierda" ? "order-2 lg:rounded-r-[12px]" : "order-1 lg:rounded-l-[12px]"}`}>
                    {props.img && Array.isArray(props.img) && props.img.map((image, index) => (
                        <img
                            key={index}
                            src={image.ruta}
                            alt={`img-${index}`}
                            className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-1000 ${index === currentImageIndex ? 'opacity-100' : 'opacity-0'} max-md:rounded-t-[16px] md:rounded-l-[16px] lg:rounded-l-[0px] ${posicion === "izquierda" ? "order-2 lg:rounded-r-[12px]" : "order-1 lg:rounded-l-[12px]"}`}
